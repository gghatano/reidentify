...
> **この文書は `fix/adversarial-p4p5` ブランチ時点の作業記録です。**
> 本ブランチは `integration/adversarial-merge` で `fix/adversarial-findings`
> と統合されました。統合時に**本文書の結論が 1 点だけ覆っています**:
>
> - `distribution_distance()` は、本文書では「分位点補間は再識別成功数が
>   下がるため不採用、mean-fill + MSE を採用」としていますが、**統合後は
>   分位点版を採用**しています。判断の根拠になった測定が
>   「RAW と ANON のレコード件数が常に一致する」データを使っており、
>   その条件では件数自体が同一性シグナルとして働くためです。件数が
>   異なる条件で測り直すと分位点版の方が 16〜46% 多く再識別できました。
>   詳細と再現手順: `docs/investigation/distance-variant-benchmark.R`
>
> それ以外の記述（`row_number` バグ、`.Rbuildignore`、`.data$` 化、
> `calc_KL` の総和正規化、NA 検出など）は統合後もそのまま有効です。
